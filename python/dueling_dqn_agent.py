...
class DuelingDQNAgent(object) :
  ...
  def __init__(self,BUFFER_SIZE,state_size, state_emb , hidden_size,action_size, batch_size, \
               gamma, optimizer, criterion, lr ,device, UPDATE_EVERY, tau, k, beta, name = 'Dueling_DDQN' ) :
    '''
    ...
    self.qnetwork_local = DuelingQNetwork( state_emb, hidden_size, action_size, name = name + '_local').to(device)
    self.qnetwork_target = DuelingQNetwork( state_emb, hidden_size, action_size, name = name + '_target').to(device)
